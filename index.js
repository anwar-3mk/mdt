...
function updateMilitaryUserStatus(userId, guildId, status) {
  if (!militaryUsers[userId]) {
    // إنشاء عسكري جديد حتى لو لم تكن الهوية موجودة بعد
    const identity = identities.find(id => id.userId === userId && id.guildId === guildId);
    // محاولة جلب اسم العضو من الديسكورد كبديل للهوية
    let fallbackName = 'غير معروف';
    try {
      const guild = client.guilds?.cache?.get(guildId);
      const member = guild?.members?.cache?.get(userId);
      fallbackName = member?.displayName || member?.user?.username || client.users?.cache?.get(userId)?.username || fallbackName;
    } catch (e) {
      // تجاهل أي أخطاء في الجلب الاحتياطي
    }
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    // معالج زر الاسم الكامل (يجب أن يكون أولاً)
    if (interaction.isButton() && interaction.customId === 'full_name') {
      try {
        console.log('🔍 معالجة زر الاسم الكامل...');
        console.log('👤 المستخدم:', interaction.user.username);
        console.log('🏠 السيرفر:', interaction.guild?.name);
        ...
        console.log('✅ إظهار مودال الاسم الكامل...');
        await interaction.showModal(modal);
        console.log('✅ تم إظهار المودال بنجاح');
        return;
      } catch (error) {
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith('accept_')) {
        // تجنب التعامل مع زر مراجعة الأكواد الجديد هنا (له معالج خاص أدناه)
        if (interaction.customId.startsWith('accept_code_req_')) {
          return;
        }
      ...
      if (interaction.customId.startsWith('reject_')) {
        // تجنب التعامل مع زر مراجعة الأكواد الجديد هنا (له معالج خاص أدناه)
        if (interaction.customId.startsWith('reject_code_req_')) {
          return;
        }
        const isMilitaryCode = interaction.customId.includes('military_code_');
        let requestId;
    ...
    // معالجة اختيارات قائمة /الادارة وعرض المودالات المناسبة
    if (interaction.isStringSelectMenu() && interaction.customId === 'admin_settings_menu') {
      const selected = interaction.values[0];

      // دالة مساعدة لبناء وإظهار مودال بنص واحد
      const showSingleInputModal = async (modalId, title, inputId, label, placeholder) => {
        const modal = new ModalBuilder()
          .setCustomId(modalId)
          .setTitle(title);
      ...
      try {
        if (selected === 'set_log_channel') {
          await showSingleInputModal('modal_set_log_channel', 'تعيين روم اللوق', 'input_log_channel', 'اكتب ID روم اللوق', 'مثال: 123456789012345678');
          return;
        }
        if (selected === 'set_review_channel') {
          await showSingleInputModal('modal_set_review_channel', 'تعيين روم المراجعة', 'input_review_channel', 'اكتب ID روم المراجعة', 'مثال: 123456789012345678');
          return;
        }
        if (selected === 'set_approval_role') {
          await showSingleInputModal('modal_set_approval_role', 'تعيين رتبة القبول والرفض', 'input_approval_role', 'اكتب ID الرتبة', 'مثال: 123456789012345678');
          return;
        }
        if (selected === 'set_police_role') {
          await showSingleInputModal('modal_set_police_role', 'تعيين رتبة الشرطة/العسكرية', 'input_police_role', 'اكتب ID الرتبة', 'مثال: 123456789012345678');
          return;
        }
        if (selected === 'set_police_admin_role') {
          await showSingleInputModal('modal_set_police_admin_role', 'تعيين رتبة مسؤول الشرطة', 'input_police_admin_role', 'اكتب ID الرتبة', 'مثال: 123456789012345678');
          return;
        }
        if (selected === 'set_create_room_channel') {
          await showSingleInputModal('modal_set_create_room_channel', 'تعيين روم إنشاء الهوية', 'input_create_room_channel', 'اكتب ID الروم', 'مثال: 123456789012345678');
          return;
        }
        if (selected === 'set_direct_military_room') {
          await showSingleInputModal('modal_set_direct_military_room', 'تعيين روم مباشرة العسكر', 'input_direct_military_room', 'اكتب ID الروم', 'مثال: 123456789012345678');
          return;
        }
...
**تم التعديل من قبل:** <@${interaction.user.id}>`)
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'military_menu') {
      ...
      if (selected === 'reset_page') {
        ...
        const menuOptions = [
          { label: 'تسجيل دخول', value: 'military_login', description: 'تسجيل دخول للعمل العسكري' },
          { label: 'تعيين الكود العسكري', value: 'set_military_code', description: 'تعيين كود عسكري جديد' },
          { label: 'نقاطي', value: 'my_military_points', description: 'عرض نقاطك العسكرية' },
          { label: 'ادارة النقاط | الاكواد العسكرية', value: 'manage_military', description: 'إدارة النظام العسكري' },
...
if (interaction.isButton() && interaction.customId.startsWith('confirm_delete_evidence_url_')) {
  ...
  return;
    ...
    // عند الضغط على زر حذف مخالفة في إدارة النظام
    if (interaction.isButton() && interaction.customId.startsWith('delete_violation_')) {
      if (!hasPoliceAdminRole(interaction.member, interaction.guildId)) {
        await interaction.reply({ content: '❌ هذا الزر مخصص فقط لمسؤول الشرطة.', ephemeral: true });
        return;
      }
      const userId = interaction.customId.replace('delete_violation_', '');
      const page = 1;
      const identity = identities.find(i => i.userId === userId && i.guildId === interaction.guildId);
      ...
      const options = pageViolations.map(v => ({
        label: `${v.name} - ${v.desc || ''}`.slice(0, 90),
        value: `delete_violation_${v.id}_${userId}_${page}`
      }));
      if (totalPages > 1) {
        options.push({ label: 'رؤية المزيد', value: `delete_violation_more_${userId}_${page+1}` });
      }
      const selectMenu = new StringSelectMenuBuilder()
        .setCustomId(`delete_violation_select_${userId}_${page}`)
        .setPlaceholder('اختر مخالفة لحذفها')
        .addOptions(options);
      const row = new ActionRowBuilder().addComponents(selectMenu);
      const embed = new EmbedBuilder()
        .setTitle('حذف مخالفة')
        .setDescription('اختر مخالفة من القائمة أدناه لحذفها')
        .setColor('#ef4444');
      await interaction.reply({ embeds: [embed], components: [row], ephemeral: true });
      return;
    }
    ...
    // صفحات حذف المخالفات
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('delete_violation_select_') && interaction.values[0].startsWith('delete_violation_more_')) {
      const parts = interaction.values[0].split('_');
      const userId = parts[3];
      const page = parseInt(parts[4]);
      const identity = identities.find(i => i.userId === userId && i.guildId === interaction.guildId);
    ...
    // تنفيذ حذف المخالفة
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('delete_violation_select_') && interaction.values[0].startsWith('delete_violation_')) {
      const parts = interaction.values[0].split('_');
        ...
        try {
          ...
          if (logChannel) {
            const logEmbed = new EmbedBuilder()
              ...
              .setDescription(`تم حذف مخالفة بواسطة: <@${interaction.user.id}>\nللمستخدم: <@${identity.userId}>\nالعنوان: ${removed.name}\nالوصف: ${removed.desc || ''}`)
              .setColor('#ef4444')
              .setTimestamp();
            await logChannel.send({ embeds: [logEmbed] });
          }
      ...
      await interaction.reply({ content: '✅ تم حذف المخالفة بنجاح.', ephemeral: true });
      return;
    }
...
if (interaction.isButton() && interaction.customId.startsWith('edit_violation_')) {
  ...
  return;
    ...
    // عند الضغط على زر حذف جريمة في إدارة النظام
    if (interaction.isButton() && interaction.customId.startsWith('delete_crime_')) {
      if (!hasPoliceAdminRole(interaction.member, interaction.guildId)) {
        await interaction.reply({ content: '❌ هذا الزر مخصص فقط لمسؤول الشرطة.', ephemeral: true });
        return;
      }
      const userId = interaction.customId.replace('delete_crime_', '');
      const page = 1;
      const identity = identities.find(i => i.userId === userId && i.guildId === interaction.guildId);
      ...
      const options = pageCrimes.map(c => ({
        label: `${c.title} - ${c.desc || ''}`.slice(0, 90),
        value: `delete_crime_${c.id}_${userId}_${page}`
      }));
      if (totalPages > 1) {
        options.push({ label: 'رؤية المزيد', value: `delete_crime_more_${userId}_${page+1}` });
      }
      const selectMenu = new StringSelectMenuBuilder()
        .setCustomId(`delete_crime_select_${userId}_${page}`)
        .setPlaceholder('اختر جريمة لحذفها')
        .addOptions(options);
      const row = new ActionRowBuilder().addComponents(selectMenu);
      const embed = new EmbedBuilder()
        .setTitle('حذف جريمة')
        .setDescription('اختر جريمة من القائمة أدناه لحذفها')
        .setColor('#ef4444');
      await interaction.reply({ embeds: [embed], components: [row], ephemeral: true });
      return;
    }
    ...
    // صفحات حذف الجرائم
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('delete_crime_select_') && interaction.values[0].startsWith('delete_crime_more_')) {
      const parts = interaction.values[0].split('_');
      const userId = parts[3];
      const page = parseInt(parts[4]);
      const identity = identities.find(i => i.userId === userId && i.guildId === interaction.guildId);
    ...
    // تنفيذ حذف الجريمة
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('delete_crime_select_') && interaction.values[0].startsWith('delete_crime_')) {
      const parts = interaction.values[0].split('_');
      ...
      if (logChannelId) {
        try {
          ...
          if (logChannel) {
            const logEmbed = new EmbedBuilder()
              .setTitle('🗑️ حذف جريمة')
              .setDescription(`تم حذف جريمة بواسطة: <@${interaction.user.id}>\nللمستخدم: <@${identity.userId}>\nالعنوان: ${removed.title}\nالوصف: ${removed.desc || ''}`)
              .setColor('#ef4444')
              .setTimestamp();
            await logChannel.send({ embeds: [logEmbed] });
          }
      ...
      await interaction.reply({ content: '✅ تم حذف الجريمة بنجاح.', ephemeral: true });
      return;
    }

    // عند الضغط على زر تعديل جريمة في إدارة النظام
    if (interaction.isButton() && interaction.customId.startsWith('edit_crime_')) {
      if (!hasPoliceAdminRole(interaction.member, interaction.guildId)) {
        await interaction.reply({ content: '❌ هذا الزر مخصص فقط لمسؤول الشرطة.', ephemeral: true });
        return;
      }
      const userId = interaction.customId.replace('edit_crime_', '');
      const page = 1;
      const identity = identities.find(i => i.userId === userId && i.guildId === interaction.guildId);
      if (!identity || !identity.crimes || identity.crimes.length === 0) {
        await interaction.reply({ content: 'لا توجد جرائم لهذا الشخص.', ephemeral: true });
      ...
      const options = pageCrimes.map(c => ({
        label: `${c.title} - ${c.desc || ''}`.slice(0, 90),
        value: `edit_crime_${c.id}_${userId}_${page}`
      }));
      if (totalPages > 1) {
        options.push({ label: 'رؤية المزيد', value: `edit_crime_more_${userId}_${page+1}` });
      }
      const selectMenu = new StringSelectMenuBuilder()
        .setCustomId(`edit_crime_select_${userId}_${page}`)
        .setPlaceholder('اختر جريمة لتعديلها')
        .addOptions(options);
      const row = new ActionRowBuilder().addComponents(selectMenu);
      const embed = new EmbedBuilder()
        .setTitle('تعديل جريمة')
        .setDescription('اختر جريمة من القائمة أدناه لتعديلها')
        .setColor('#3b82f6');
      await interaction.reply({ embeds: [embed], components: [row], ephemeral: true });
      return;
    }
    ...
    // صفحات تعديل الجرائم
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('edit_crime_select_') && interaction.values[0].startsWith('edit_crime_more_')) {
      const parts = interaction.values[0].split('_');
      const userId = parts[3];
      const page = parseInt(parts[4]);
      const identity = identities.find(i => i.userId === userId && i.guildId === interaction.guildId);
    ...
    // اختيار جريمة للتعديل ثم عرض أزرار الحالة
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('edit_crime_select_') && interaction.values[0].startsWith('edit_crime_')) {
      const parts = interaction.values[0].split('_');
      const crimeId = parts[2];
      const userId = parts[3];
      const identity = identities.find(i => i.userId === userId && i.guildId === interaction.guildId);
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`set_crime_status_${userId}_${crimeId}_done`).setLabel('منتهية').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId(`set_crime_status_${userId}_${crimeId}_undone`).setLabel('غير منتهية').setStyle(ButtonStyle.Danger)
      );
      const embed = new EmbedBuilder()
        .setTitle('تعديل حالة الجريمة')
        .setDescription(`العنوان: ${identity.crimes[idx].title}\nالوصف: ${identity.crimes[idx].desc || ''}\nالحالة الحالية: ${identity.crimes[idx].done ? 'منتهية' : 'غير منتهية'}`)
        .setColor('#3b82f6');
      await interaction.reply({ embeds: [embed], components: [row], ephemeral: true });
      return;
    }

    // تنفيذ تعديل حالة الجريمة
    if (interaction.isButton() && interaction.customId.startsWith('set_crime_status_')) {
      const parts = interaction.customId.split('_');
      const userId = parts[3];
      const crimeId = parts[4];
      const done = parts[5] === 'done';
      const identity = identities.find(i => i.userId === userId && i.guildId === interaction.guildId);
      const idx = identity && identity.crimes ? identity.crimes.findIndex(c => c.id === crimeId) : -1;
      if (!identity || idx === -1) {
        await interaction.reply({ content: 'تعذر العثور على الجريمة.', ephemeral: true });
      ...
      if (logChannelId) {
        try {
          ...
          if (logChannel) {
            const logEmbed = new EmbedBuilder()
              .setTitle('✏️ تعديل حالة جريمة')
              .setDescription(`تم تعديل حالة جريمة بواسطة: <@${interaction.user.id}>\nللمستخدم: <@${identity.userId}>\nالعنوان: ${identity.crimes[idx].title}\nالحالة الجديدة: ${done ? 'منتهية' : 'غير منتهية'}`)
              .setColor(done ? '#00ff00' : '#ff0000')
              .setTimestamp();
            await logChannel.send({ embeds: [logEmbed] });
          }
        } catch (e) { /* تجاهل الخطأ */ }
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`set_crime_status_${userId}_${crimeId}_done`).setLabel('منتهية').setStyle(ButtonStyle.Success).setDisabled(true),
        new ButtonBuilder().setCustomId(`set_crime_status_${userId}_${crimeId}_undone`).setLabel('غير منتهية').setStyle(ButtonStyle.Danger).setDisabled(true)
      );
      const embed = new EmbedBuilder()
        .setTitle('تعديل حالة الجريمة')
        .setDescription(`العنوان: ${identity.crimes[idx].title}\nالحالة الجديدة: ${done ? 'منتهية' : 'غير منتهية'}`)
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'dev_menu' && interaction.values[0] === 'change_embed') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('اختر سيرفر من القائمة أدناه لتغيير صورة الإمبد الخاصة به. سيتم تطبيق التغيير على جميع الأوامر في السيرفر المحدد.')
        .setColor('#00ff00');
      await interaction.reply({ embeds: [embed], components: [row], ephemeral: true });
      return;
    }
...
**تم التعديل من قبل:** <@${interaction.user.id}>`)
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('dev_guilds_menu_') && interaction.values[0].startsWith('dev_select_guild_')) {
      ...
      try {
        ...
        // إضافة صورة الإمبد المخصصة إذا كانت موجودة
        const customEmbedImage = guildSettings[guildId]?.customEmbedImage;
        if (customEmbedImage) {
          embed.setImage(customEmbedImage);
        } else if (iconURL) {
          embed.setImage(iconURL);
        }
        // التحقق من وجود إمبد مخصص
        const hasCustomEmbed = guildSettings[guildId]?.customEmbedImage;
        ...
        embed.addFields(
          ...
          { name: 'صورة الإمبد', value: hasCustomEmbed ? '🟢 مخصصة' : '⚪ افتراضية', inline: true },
          { name: 'رابط السيرفر', value: invite || 'لا يوجد دعوة متاحة', inline: false }
        );
        // زر تغيير ايمبيد وزر إعادة تعيين الايمبيد
        const changeEmbedBtn = new ButtonBuilder()
          .setCustomId(`dev_change_embed_${guild.id}`)
          .setLabel('تغيير الإمبد')
          .setStyle(ButtonStyle.Primary);
        const resetEmbedBtn = new ButtonBuilder()
          .setCustomId(`dev_reset_embed_${guild.id}`)
          .setLabel('إعادة تعيين الإمبد')
          .setStyle(ButtonStyle.Secondary);
        // إضافة زر العودة لقائمة السيرفرات
        const backButton = new ButtonBuilder()
          .setCustomId('dev_back_to_guilds')
          .setLabel('العودة لقائمة السيرفرات')
          .setStyle(ButtonStyle.Secondary);
    ...
    if (interaction.isButton() && interaction.customId.startsWith('dev_change_embed_')) {
      ...
      // مودال لإدخال رابط صورة الإمبد
      const modal = new ModalBuilder()
        .setCustomId(`dev_modal_embed_url_${guildId}`)
        .setTitle('تغيير صورة الإمبد');
      const input = new TextInputBuilder()
        .setCustomId('input_embed_url')
        .setLabel('رابط صورة الإمبد')
        .setStyle(TextInputStyle.Short)
        .setPlaceholder('ضع هنا رابط صورة الإمبد (jpg/png/gif/webp)')
        .setRequired(true)
        .setMaxLength(300);
      const row = new ActionRowBuilder().addComponents(input);
      modal.addComponents(row);
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('dev_modal_embed_url_')) {
      ...
      const guild = client.guilds.cache.get(guildId);
      const guildName = guild ? guild.name : 'السيرفر المحدد';
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'المطور') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('مرحباً بك في قائمة أوامر المطور. اختر من القائمة أدناه الإجراء المطلوب.')
        .setImage(customImage)
        .setColor('#00ff00');
    ...
    if (interaction.isButton() && interaction.customId.startsWith('toggle_bot_status_')) {
      ...
      const embed = new EmbedBuilder()
        ...
        if (newStatus === 'offline') {
          embed.addFields(
            ...
            { name: '👤 تم التغيير بواسطة', value: `<@${interaction.user.id}> (\`${interaction.user.username}\`)`, inline: true },
            { name: '⏰ وقت التغيير', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
            { name: '🕐 وقت التغيير (نسبي)', value: `<t:${Math.floor(Date.now() / 1000)}:R>`, inline: true }
          );
        } else {
          embed.addFields(
            { name: '👤 تم التغيير بواسطة', value: `<@${interaction.user.id}> (\`${interaction.user.username}\`)`, inline: true },
        ...
        // إضافة footer للإمبد
        embed.setFooter({ 
          text: `MDT Bot Status Change • ${newStatus === 'online' ? '🟢 متصل' : '🔴 غير متصل'}`,
      ...
      // إعداد النص حسب حالة البوت
      const statusText = newStatus === 'online' ? 'تشغيل' : 'إيقاف';
      const actionText = newStatus === 'online' ? 'تم تشغيل البوت بسيرفر' : 'لقد تم إيقاف البوت عن سيرفر';
      const embedColor = newStatus === 'online' ? '#00ff00' : '#ff0000';
      const embedTitle = newStatus === 'online' ? '🟢 تم تشغيل البوت' : '🔴 تم إيقاف البوت';
      
      // محاولة الحصول على رابط الدعوة للسيرفر
      let inviteLink = 'غير متوفر';
      try {
        ...
        if (firstInvite) {
          ...
          // إذا لم توجد دعوات، حاول إنشاء واحدة
        ...
        console.log('❌ فشل في الحصول على رابط الدعوة:', error.message);
        inviteLink = 'غير متوفر';
      }
        ...
        for (const developerId of DEVELOPER_IDS) {
          try {
            ...
            if (developer) {
              ...
              // إرسال الإشعار لجميع المطورين (بما فيهم المطور الذي قام بالتغيير)
                              const notificationEmbed = new EmbedBuilder()
                  .setTitle(embedTitle)
                  .setDescription(`**${actionText} \`${guild.name}\` من قبل \`${interaction.user.username}\`**`)
                  .addFields(
                    { name: '👤 **المطور المسؤول**', value: `<@${interaction.user.id}> (\`${interaction.user.username}\`)`, inline: true },
                    { name: '🏠 **السيرفر**', value: `\`${guild.name}\``, inline: true },
                    { name: '🆔 **ايدي السيرفر**', value: `\`${guild.id}\``, inline: true },
                    { name: '📊 **عدد الأعضاء**', value: `\`${guild.memberCount} عضو\``, inline: true },
                    { name: '🔗 **رابط السيرفر**', value: inviteLink !== 'غير متوفر' ? `[انقر هنا للدخول](${inviteLink})` : '`غير متوفر`', inline: true },
                    { name: '⏰ **وقت التغيير**', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true },
                    { name: '🕐 **وقت التغيير (نسبي)**', value: `<t:${Math.floor(Date.now() / 1000)}:R>`, inline: true }
                  )
                  .setColor(embedColor)
        ...
        setTimeout(async () => {
          try {
            const confirmEmbed = new EmbedBuilder()
              .setTitle(`✅ تم ${newStatus === 'online' ? 'تشغيل' : 'إيقاف'} البوت بنجاح`)
              .setDescription(`**تم ${newStatus === 'online' ? 'تشغيل' : 'إيقاف'} البوت بنجاح في سيرفر \`${guild.name}\`**\n\n📋 **التفاصيل:**\n• **السيرفر:** \`${guild.name}\`\n• **ايدي السيرفر:** \`${guild.id}\`\n• **عدد الأعضاء:** \`${guild.memberCount} عضو\`\n• **رابط السيرفر:** ${inviteLink !== 'غير متوفر' ? `[انقر هنا للدخول](${inviteLink})` : '`غير متوفر`'}\n• **تم التغيير بواسطة:** \`${interaction.user.username}\`\n• **وقت التغيير:** <t:${Math.floor(Date.now() / 1000)}:F>\n\n💡 **ملاحظة:** تم إرسال إشعار لجميع المطورين المصرح لهم.`)
              .setColor(newStatus === 'online' ? '#00ff00' : '#ff0000')
              .setThumbnail(guild.iconURL({ dynamic: true }))
              .setFooter({ 
                text: `MDT Bot Status Change • ${newStatus === 'online' ? '🟢 متصل' : '🔴 غير متصل'}`,
            ...
            console.error('خطأ في إرسال رسالة التأكيد:', followUpError);
          }
        }, 1000);
        
      } catch (error) {
        console.error('خطأ في تحديث التفاعل:', error);
        // إذا فشل التحديث، حاول إرسال رسالة جديدة
        try {
          const errorConfirmEmbed = new EmbedBuilder()
            .setTitle(`✅ تم ${newStatus === 'online' ? 'تشغيل' : 'إيقاف'} البوت بنجاح`)
            .setDescription(`**تم ${newStatus === 'online' ? 'تشغيل' : 'إيقاف'} البوت بنجاح في سيرفر \`${guild.name}\`**\n\n📋 **التفاصيل:**\n• **السيرفر:** \`${guild.name}\`\n• **ايدي السيرفر:** \`${guild.id}\`\n• **عدد الأعضاء:** \`${guild.memberCount} عضو\`\n• **رابط السيرفر:** ${inviteLink !== 'غير متوفر' ? `[انقر هنا للدخول](${inviteLink})` : '`غير متوفر`'}\n• **تم التغيير بواسطة:** \`${interaction.user.username}\`\n• **وقت التغيير:** <t:${Math.floor(Date.now() / 1000)}:F>\n\n💡 **ملاحظة:** تم إرسال إشعار لجميع المطورين المصرح لهم.`)
            .setColor(newStatus === 'online' ? '#00ff00' : '#ff0000')
            .setThumbnail(guild.iconURL({ dynamic: true }))
          ...
          await interaction.followUp({ 
            embeds: [errorConfirmEmbed], 
            ephemeral: true 
          });
        } catch (followUpError) {
          console.error('خطأ في إرسال الرسالة البديلة:', followUpError);
        }
      }
      return;
    }

    // عند الضغط على زر العودة لقائمة السيرفرات
    if (interaction.isButton() && interaction.customId === 'dev_back_to_guilds') {
      // تحقق من أن المستخدم مطور مصرح له
      if (!isDeveloper(interaction.user.id)) {
        await interaction.followUp({ 
          content: '❌ هذا الأمر مخصص فقط للمطورين المصرح لهم.', 
    ...
    if (interaction.isButton() && interaction.customId.startsWith('dev_reset_embed_')) {
      ...
      if (guildSettings[guildId] && guildSettings[guildId].customEmbedImage) {
        ...
        await interaction.followUp({ 
          content: `✅ تم إعادة تعيين صورة الإمبد إلى الافتراضية لسيرفر **${guildName}**!\n\n📋 **التفاصيل:**\n• تم إعادة تعيين صورة الإمبد إلى الصورة الافتراضية\n• سيتم تطبيق التغيير على جميع الأوامر في السيرفر\n• يمكنك تغيير الإمبد مرة أخرى من زر "تغيير الإمبد"\n\n💡 **ملاحظة:** التغيير سيظهر في جميع الأوامر مثل /بطاقة، /الادارة، /هويتي، /الشرطة، /العسكر، /المطور\n\n🎯 **الأوامر المتأثرة:**\n• /بطاقة - إنشاء بطاقة هوية\n• /الادارة - إدارة الهويات\n• /هويتي - عرض هويتك\n• /الشرطة - أوامر الشرطة\n• /العسكر - نظام العسكر\n• /المطور - أوامر المطور`, 
          ephemeral: true 
        });
      } else {
        // جلب اسم السيرفر للرسالة
        ...
        await interaction.followUp({ 
          content: `❗️ لا يوجد صورة مخصصة حالياً لسيرفر **${guildName}**.\n\n📋 **معلومات:**\n• السيرفر يستخدم الصورة الافتراضية\n• يمكنك تغيير الإمبد من زر "تغيير الإمبد"\n\n💡 **ملاحظة:** التغيير سيظهر في جميع الأوامر مثل /بطاقة، /الادارة، /هويتي، /الشرطة، /العسكر، /المطور\n\n🎯 **الأوامر المتأثرة:**\n• /بطاقة - إنشاء بطاقة هوية\n• /الادارة - إدارة الهويات\n• /هويتي - عرض هويتك\n• /الشرطة - أوامر الشرطة\n• /العسكر - نظام العسكر\n• /المطور - أوامر المطور`, 
          ephemeral: true 
        });
      }
      return;
    ...
    if (interaction.isButton() && interaction.customId.startsWith('delete_pending_military_code_')) {
      ...
      if (!pendingRequest) {
        await interaction.followUp({ content: '❌ لم يتم العثور على الطلب المعلق.', ephemeral: true });
        return;
      }
      ...
      await interaction.followUp({ content: '✅ تم حذف الطلب المعلق بنجاح! يمكن للعسكري تقديم طلب جديد.', ephemeral: true });
      return;
    }
    ...
    if (interaction.isButton() && interaction.customId.startsWith('confirm_add_points_')) {
      ...
      try {
        ...
        await interaction.followUp({ content: '❌ خطأ في إضافة النقاط.', ephemeral: true });
      }
      return;
    }
    ...
    if (interaction.isButton() && interaction.customId.startsWith('confirm_remove_points_')) {
      ...
      try {
        ...
        await interaction.followUp({ content: '❌ خطأ في خصم النقاط.', ephemeral: true });
      }
      return;
    }
