...
client.on('interactionCreate', async interaction => {
  try {
    ...
    // معالج قائمة اختيار اليوم (الخطوة الأخيرة)
...
**تم التعديل من قبل:** <@${interaction.user.id}>`)
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'modal_manage_military_codes') {
      ...
      if (!foundIdentity) {
        await interaction.followUp({ content: '❌ لم يتم العثور على شخص بهذا الاسم أو الرقم الوطني.', ephemeral: true });
        return;
      }
      ...
      try {
        ...
        await interaction.followUp({ content: '❌ خطأ في جلب معلومات المستخدم.', ephemeral: true });
      }
      return;
    }
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('modal_edit_military_code_')) {
      ...
      if (!newCode || newCode.trim() === '') {
        await interaction.followUp({ content: '❌ يرجى إدخال كود عسكري صحيح.', ephemeral: true });
        return;
      }
